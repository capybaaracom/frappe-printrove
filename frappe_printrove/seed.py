...
TEMPLATE_STATIC_ATTRIBUTES = {
	"Half Sleeve Round Neck T-Shirt": {
		"Fit": "Regular Fit, Boxy",
		"Neckline": "Crew Neck",
		"Sleeve": "Half Sleeves, Hemmed Cuff",
		"Hemline": "Straight",
		"Material": "Cotton, GSM 180",
	},
	"Polo T-shirts": {
		"Fit": "Regular Fit",
		"Neckline": "Polo",
...
def ensure_item_attribute(attr_name, attr_values):
	...
	existing_vals = {
		row.attribute_value.lower(): row.abbr for row in doc.item_attribute_values if row.attribute_value
	}
	...
	for val in sorted(list(attr_values)):
		...
		if val.lower() not in existing_vals:
			...
			doc.append("item_attribute_values", {"attribute_value": val, "abbr": abbr})
...
def ensure_supplier_item(doc, supplier, part_no):
	...
	if not found:
		doc.append("supplier_items", {"supplier": supplier, "supplier_part_no": part_no})
		changed = True
	return changed
...
def run():
	...
	for t_data in templates_data:
		...
		# Otherwise, the template gets the product ID, and variants get their own IDs
		...
		if existing_item:
			# Update existing item's printrove_id and supplier part no if needed
			# We use db_set to avoid triggering validations and overwriting other fields like tags/attributes
			frappe.db.set_value(
				"Item",
				existing_item,
				{
					"printrove_id": main_printrove_id,
					"printrove_category_id": str(t_data["category_id"]),
					"printrove_base_product_id": str(t_data["template_id"]),
					"delivered_by_supplier": 1,
				},
				update_modified=False,
			)

			# Ensure supplier item exists
			item_doc = frappe.get_doc("Item", existing_item)
			if ensure_supplier_item(item_doc, "Printrove Products Private Limited", main_printrove_id):
			...
			doc.printrove_category_id = str(t_data["category_id"])
			...
			if has_variant_attr:
				for attr, val in t_data["attributes"].items():
					...
					doc.append(
						"attributes",
						{"attribute": attr, "attribute_value": val, "include_in_item_code": include_in_code},
					)
			else:
		...
		for variant in t_data["variants"]:
			...
			if existing_variant:
				# Update existing variant's details if needed
				frappe.db.set_value(
					"Item",
					existing_variant,
					{"delivered_by_supplier": 1, "valuation_rate": variant.get("base_price", 0)},
					update_modified=False,
				)
				...
				if variant.get("weight"):
					frappe.db.set_value(
						"Item",
						existing_variant,
						{"weight_per_unit": variant.get("weight") / 1000.0, "weight_uom": "Kg"},
						update_modified=False,
					)
