...
| Design API (`/api/external/designs/url`) | HTTPS / POST | JSON (`DesignUrlRequest`) | Outbound | Submits public artwork image URLs for asynchronous rendering and storage. |
| Product API (`/api/external/products`) | HTTPS / POST | JSON (`ProductCreateRequest`) | Outbound | Registers custom SKU configurations mapping artwork coordinates to blank garments. |
| Serviceability API (`/api/external/serviceability`) | HTTPS / GET | Query Params (`ServiceabilityRequest`) | Outbound | Inquires courier feasibility and real-time shipping costs based on pincode and weight. |
| Order API (`/api/external/orders`) | HTTPS / POST | JSON (`OrderCreateRequest`) | Outbound | Dispatches purchase order, shipping details, and SKU items for production. |
